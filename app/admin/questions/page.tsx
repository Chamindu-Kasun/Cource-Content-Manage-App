...
import { Question, Topic, Unit } from '@/types';
import Link from 'next/link';
import { TrashIcon, PencilIcon, PlusIcon, QuestionMarkCircleIcon } from '@heroicons/react/24/outline';
...
export default function QuestionsPage() {
  ...
  const loadData = async () => {
    try {
      const [questionsSnap, topicsSnap, unitsSnap] = await Promise.all([
      ...
      const questionsData: QuestionWithDetails[] = [];
  ...
  return (
    <div className="space-y-6">
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold leading-6 text-gray-900">Questions</h1>
          <p className="mt-2 text-sm text-gray-700">
            Manage practice questions organized by Unit → Topic structure
          </p>
        </div>
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        {/* Add Question Button - only show when topic is selected */}
        {selectedTopic && (
          <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-sm font-medium text-green-900">Ready to add practice question</h4>
                <p className="text-sm text-green-700">
                  Selected: {units.find(u => u.id === selectedUnit)?.unit_title} → {filteredTopics.find(t => t.id === selectedTopic)?.topic_title}
                </p>
              </div>
              <Link
                href={`/admin/questions/create?unit=${selectedUnit}&topic=${selectedTopic}`}
      ...
      {filteredQuestions.length === 0 ? (
        <div className="text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">
            {selectedTopic ? 'No questions for this topic' : selectedUnit ? 'No questions in this unit' : 'No questions'}
          </h3>
          <p className="mt-1 text-sm text-gray-500">
            {selectedTopic 
              ? 'Get started by creating a practice question for this topic.' 
              : selectedUnit
              ? 'No questions found for the selected unit. Select a topic to add questions.'
              : 'Select a unit and topic above to add practice questions.'}
          </p>
          {selectedTopic && (
            <div className="mt-6">
              <Link
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <div className="text-sm text-gray-500 mt-1">
                        {question.unit_title} - {question.topic_title}
                      </div>
